...
export default function DetailKontrakView() {
    ...
    const { kontrak, loading, isUpdating, setujuiKontrak, tolakKontrak } = useAdminKontrak();
    ...
    const handleApprove = async () => {
        ...
        const confirmApprove = window.confirm('Setujui kontrak ini? Status penghuni akan diubah menjadi Aktif.');
        if (confirmApprove) {
    ...
    if (loading || !selectedKontrak) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh]">
                ...
                <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Memuat Detail Kontrak...</p>
            </div>
        );
    }
    ...
    return (
        <div className="px-5 mt-6 space-y-5 pb-10 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-lg font-black text-[#0D2F5C] uppercase tracking-widest">Detail Kontrak</h1>
                    <p className="text-[#7A93B5] text-xs font-medium mt-0.5">Kamar {selectedKontrak.user?.room?.room_number}</p>
                </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-3xl shadow-[0_4px_20px_rgba(13,47,92,0.05)] border border-slate-100 space-y-6">
                ...
                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Status Kontrak</p>
                        <p className={`text-sm font-black uppercase tracking-widest ${selectedKontrak.status === 'menunggu_persetujuan' ? 'text-orange-500' : selectedKontrak.status === 'aktif' ? 'text-emerald-500' : 'text-rose-500'}`}>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2 text-[#0D2F5C] border-b border-slate-100 pb-2">
                        ...
                        <h3 className="text-xs font-black uppercase tracking-widest">Informasi Pengajuan</h3>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Penghuni</p>
                            <p className="font-bold text-[#0D2F5C] text-sm">{selectedKontrak.user?.profile?.nama_lengkap || '-'}</p>
                        </div>
                        <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Jenis Kontrak</p>
                            <p className="font-bold text-blue-600 text-sm capitalize">{selectedKontrak.jenis_kontrak}</p>
                        </div>
                        <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Durasi Sewa</p>
                            <p className="font-bold text-slate-700 text-sm">{selectedKontrak.lama_sewa} Bulan</p>
                        </div>
                        <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Biaya / Bulan</p>
                            <p className="font-black text-emerald-600 text-sm">{formatCurrency(selectedKontrak.harga_per_bulan)}</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-blue-50 p-3.5 rounded-2xl border border-blue-100">
                            <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest mb-1">Mulai Masuk</p>
                            <p className="font-bold text-blue-800 text-sm">{formatDate(selectedKontrak.mulai_sewa)}</p>
                        </div>
                        <div className="bg-rose-50 p-3.5 rounded-2xl border border-rose-100">
                            <p className="text-[9px] font-black text-rose-400 uppercase tracking-widest mb-1">Akhir Kontrak</p>
                            <p className="font-bold text-rose-800 text-sm">{formatDate(selectedKontrak.akhir_sewa)}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-[#0D2F5C] text-white">
                            <h2 className="text-sm font-black uppercase tracking-widest">Tolak Kontrak</h2>
                            <button 
                                title="Tutup Modal"
                        ...
                        <div className="p-5 space-y-4">
                            <div className="bg-rose-50 text-rose-800 p-3.5 rounded-2xl text-xs border border-rose-100 font-medium leading-relaxed">
                                Penolakan kontrak atas nama <strong>{selectedKontrak.user?.profile?.nama_lengkap}</strong>.
                            </div>
