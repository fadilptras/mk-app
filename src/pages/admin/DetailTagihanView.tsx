...
export default function DetailTagihanView() {
    ...
    if (loading || !selectedTagihan) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh]">
                ...
                <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Memuat Detail Tagihan...</p>
            </div>
        );
    }
    ...
    const getStatusDisplay = (status: string) => {
        switch(status) {
            case 'paid': return { label: 'Lunas', color: 'text-emerald-500' };
            case 'pending': return { label: 'Dibayar (Menunggu Verifikasi)', color: 'text-amber-500' };
            case 'unpaid': return { label: 'Belum Bayar', color: 'text-rose-500' };
        ...
        <div className="px-5 mt-6 space-y-5 pb-10">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-[#7A93B5] text-xs font-medium mt-0.5">Kamar {selectedTagihan.user?.room?.room_number}</p>
                </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-3xl shadow-[0_4px_20px_rgba(13,47,92,0.05)] border border-slate-100 space-y-6">
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <p className={`text-sm font-black uppercase tracking-widest ${statusInfo.color}`}>
                        {statusInfo.label}
                    </p>
                </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2 text-[#0D2F5C] border-b border-slate-100 pb-2">
                        ...
                        <h3 className="text-xs font-black uppercase tracking-widest">Informasi Tagihan</h3>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Penghuni</p>
                            <p className="font-bold text-[#0D2F5C] text-sm">{selectedTagihan.user?.profile?.nama_lengkap || '-'}</p>
                        </div>
                        <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Periode</p>
                            <p className="font-bold text-[#0D2F5C] text-sm">{selectedTagihan.periode_tagihan}</p>
                        </div>
                        <div className="col-span-2 bg-slate-50 p-3.5 rounded-2xl border border-slate-100 flex justify-between items-center">
                            <div>
                                ...
                                <p className="font-bold text-rose-600 text-sm">{formatDate(selectedTagihan.jatuh_tempo)}</p>
                            </div>
                ...
                <div className="space-y-3">
                    <p className="text-slate-500 uppercase tracking-widest text-[10px] font-black mb-2">Bukti Transfer Penghuni</p>
                    <div className="bg-slate-100 rounded-2xl p-2 border border-slate-200 min-h-[250px] flex items-center justify-center overflow-hidden">
                        {selectedTagihan.bukti_transfer ? (
                            <a href={selectedTagihan.bukti_transfer} target="_blank" rel="noreferrer" className="block w-full text-center">
                                <img 
                                ...
                                />
                                <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-3">Klik gambar untuk memperbesar</p>
                            </a>
                        ) : (
                            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Belum Ada Lampiran</p>
                        )}
                    </div>
                </div>
                ...
                {selectedTagihan.keterangan && (
                    <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl">
                        <p className="text-amber-800 text-xs font-medium leading-relaxed">
                            <span className="font-black uppercase tracking-widest text-[9px] block mb-1.5">Catatan Penghuni saat bayar:</span> 
                            "{selectedTagihan.keterangan}"
                        </p>
                    </div>
                )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-[#0D2F5C] text-white">
                            ...
                            <button onClick={() => setRejectModalOpen(false)} className="text-white/70 hover:text-white">
                        ...
                        <div className="p-5 space-y-4">
                            <div className="bg-rose-50 text-rose-800 p-3.5 rounded-2xl text-xs border border-rose-100 font-medium leading-relaxed">
                                Anda akan menolak pembayaran tagihan dari <strong>{selectedTagihan.user?.profile?.nama_lengkap}</strong>. Status akan dikembalikan menjadi Belum Bayar.
                            </div>
