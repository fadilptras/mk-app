...
export default function KelolaLaporanView() {
  ...
  return (
    <div className="px-5 mt-6 space-y-4">
      ...
      {loading ? (
        <p className="text-center text-xs py-10 text-slate-400">
          Memuat data laporan...
        </p>
      ) : reports.length === 0 ? (
        ...
        <div className="space-y-4">
          {reports.map((report) => {
            ...
            return (
              <div
                key={report.id}
                className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-col space-y-4"
              >
                {/* Header Laporan */}
                <div className="flex justify-between items-start border-b border-slate-50 pb-3">
                  ...
                  <span
                    className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase border ${getStatusColor(report.status)}`}
                  >
