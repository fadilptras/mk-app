...
export default function KelolaTagihanView() {
  ...
  const getStatusDisplay = (status: string) => {
    switch (status) {
      ...
      default:
        return { label: status, color: "bg-slate-50 text-slate-600" };
    }
  };
    ...
    <div className="px-5 mt-6 space-y-5 pb-10">
      <div>
        ...
        <p className="text-[#7A93B5] text-xs font-medium mt-1">
          Pantau pembayaran sewa penghuni
        </p>
      </div>
        ...
        {loading ? (
          ...
          filteredTagihan.map((item) => {
            ...
            return (
              ...
              >
                <div
                  className={`absolute top-0 left-0 w-1.5 h-full ${item.status === "paid" ? "bg-emerald-500" : "bg-rose-500"}`}
                ></div>
                ...
                <div className="flex justify-between items-center pl-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider border ${statusInfo.color}`}
                    >
                      {statusInfo.label}
                    </span>
