...
export default function SewaView() {
  ...
  const { activeContract, getRemainingDays, loading: contractLoading } = useKontrak();
  ...
  const contractEndDate = activeContract 
    ? new Date(activeContract.akhir_sewa).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })
    : "-";
        ...
        <div className="p-5 space-y-6">
          ...
          {!isContractComplete ? (
            <div className="bg-gradient-to-r from-orange-500 to-amber-500 rounded-[20px] p-4 shadow-lg shadow-orange-200 relative overflow-hidden flex items-center justify-between">
              ...
              <button onClick={() => navigate('/kontrak/perpanjang')} className="relative z-10 bg-white text-orange-600 text-[10px] font-black px-4 py-2.5 rounded-xl shadow-sm transition-all active:scale-95 whitespace-nowrap">
                BUAT KONTRAK
              </button>
            </div>
          ) : (
            <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5 ml-2">
                <button onClick={() => navigate('/kontrak')} className="bg-indigo-50 text-indigo-600 hover:bg-indigo-100 p-2 rounded-lg transition-all active:scale-95 flex items-center justify-center shrink-0" title="Dashboard Kontrak">
                ...
                <button disabled={!canRenew} onClick={() => navigate('/kontrak/perpanjang')} className={`${canRenew ? 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100' : 'bg-gray-100 text-gray-400 cursor-not-allowed'} text-[10px] font-black px-3 py-2 rounded-lg transition-all active:scale-95 flex items-center gap-1.5 whitespace-nowrap`}>
            ...
            {!isContractComplete ? (
              <div className="text-center py-6 relative z-10">
                ...
                <p className="text-xs text-gray-500 px-4 leading-relaxed">
                  Silakan ajukan atau selesaikan pengisian kontrak terlebih dahulu untuk dapat menerbitkan tagihan kamar Anda.
                </p>
              ...
              <>
                <div className="flex justify-between items-start mb-5 relative z-10">
                  <div>
                    ...
                    <h3 className="text-gray-500 text-xs font-medium">Periode: {tagihanAktif.periode_tagihan}</h3>
                  </div>
                  <span className={`text-[10px] font-black px-2.5 py-1 rounded-md uppercase tracking-wider border shadow-sm ${tagihanAktif.status === 'pending' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                    {tagihanAktif.status === 'pending' ? 'Diproses' : 'Belum Bayar'}
                  </span>
                </div>
                
                <div className="mb-6 relative z-10">
                  <p className="text-3xl font-black text-gray-800 tracking-tight">{formatRupiah(tagihanAktif.nominal_tagihan)}</p>
                  <div className="flex items-center gap-1.5 mt-2 text-gray-500">
                    ...
                    <p className="text-[11px] font-bold">Jatuh Tempo: <span className="text-gray-700">{formatDate(tagihanAktif.jatuh_tempo)}</span></p>
                  </div>
                </div>
                ...
                {tagihanAktif.status === 'pending' ? (
                  ...
                  <button 
                    onClick={() => navigate('/sewa/bayar', { state: { tagihan: tagihanAktif } })}
              ...
              <div className="text-center py-6 relative z-10">
                <div className="w-16 h-16 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-3">
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>
                </div>
                <h3 className="text-sm font-black text-gray-800 uppercase mb-1">Semua Tagihan Lunas</h3>
                <p className="text-xs text-gray-500">Saat ini tidak ada beban tagihan yang harus dibayar.</p>
              </div>
            )}
          </div>
                ...
                {riwayatTagihan.map((item) => (
                  ...
                  >
                    <div className="flex gap-3 items-center">
                      ...
                      <div>
                        <p className="font-bold text-xs text-gray-800">Sewa {item.periode_tagihan}</p>
                        <p className="text-[9px] text-gray-400 font-bold uppercase mt-0.5 tracking-wider">{formatDate(item.updated_at || item.created_at)}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-black text-xs text-gray-800">{formatRupiah(item.nominal_tagihan)}</p>
