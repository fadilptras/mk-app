  ...
  const handleExportPDF = (contract: any) => {
    ...
    setTimeout(() => {
      ...
      const opt = {
        ...
        // PERBAIKAN TYPESCRIPT: Menambahkan 'as const' pada orientation
        ...
        <section className="space-y-3">
          ...
          {activeContract ? (
                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <button
                    ...
                    className={`w-full text-xs font-black py-3.5 rounded-xl transition-all uppercase tracking-wider flex items-center justify-center gap-2 ${
                      canRenew ? "bg-indigo-600 hover:bg-indigo-700 text-white shadow-md shadow-indigo-200 active:scale-95" : "bg-slate-200 text-slate-400 cursor-not-allowed border border-slate-300"
                    }`}
                  >
            ...
            <div className="bg-white p-6 rounded-[24px] border border-dashed border-slate-300 text-center shadow-md">
              ...
              <button onClick={() => navigate("/kontrak/perpanjang")} className="bg-indigo-600 hover:bg-indigo-700 text-white text-[11px] font-black py-3 px-5 rounded-xl transition-all uppercase tracking-wide">
            ...
            <div className="space-y-2.5">
              {contractHistory.map((contract: any) => (
                <div key={contract.id} className="bg-white p-4 rounded-2xl border border-white shadow-sm flex justify-between items-center gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[9px] font-mono font-bold text-slate-400">
                        {generateRefID('kontrak', contract.id, contract.created_at)}
                      </span>
                    </div>
                  ...
                  <button onClick={() => handleExportPDF(contract)} className="bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 font-black text-[9px] uppercase tracking-wide px-3 py-2 rounded-xl transition-colors flex items-center gap-1 shrink-0">
