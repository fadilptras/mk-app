...
export default function FormBayarView() {
  ...
  const handleSubmitPembayaran = async () => {
    ...
    try {
      ...
      const { data: { publicUrl } } = supabase.storage.from("bukti-transfer").getPublicUrl(filePath);
    ...
    <div className="min-h-screen bg-[#BFDDF0] font-sans text-gray-800 pb-10">
      <div className="max-w-md mx-auto bg-[#BFDDF0] min-h-screen shadow-inner">
        ...
        <div className="bg-indigo-600 px-5 py-4 flex items-center gap-4 sticky top-0 z-20 shadow-md">
          {/* FIX AXE: Menambah label aksesibilitas untuk tombol header kembali */}
          <button
            onClick={() => navigate(-1)}
            aria-label="Kembali"
            title="Kembali"
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="bg-gradient-to-br from-indigo-900 to-slate-900 text-white rounded-[24px] p-5 shadow-xl space-y-4 relative overflow-hidden">
            ...
            <div>
              ...
              <h3 className="text-lg font-black tracking-wide mt-1 uppercase text-yellow-300">
                {profileData?.nama_rek_pembayaran || (<span className="text-red-400 italic text-sm font-medium">Belum diatur Admin</span>)}
              </h3>
            </div>
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono font-black text-sm text-white tracking-wider">
                  {profileData?.no_rek_pembayaran || (<span className="text-red-400 italic text-[11px]">Belum diatur Admin</span>)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-400 font-medium">Bank Tujuan</span>
                <span className="font-bold uppercase tracking-wide text-white">
                  {profileData?.no_rek_pembayaran ? ("Bank Transfer") : (<span className="text-red-400 italic text-[11px]">Belum diatur Admin</span>)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[24px] p-5 shadow-md border border-white/60 space-y-3.5">
            ...
            <div className="space-y-2.5 text-xs font-medium text-gray-700">
              <div className="flex justify-between">
                ...
                <span className="font-black text-gray-900">{profileData?.nama_lengkap || profileData?.full_name}</span>
              </div>
              <div className="w-full h-px bg-gray-50"></div>
              <div className="flex justify-between">
                <span className="text-gray-400">Nomor Kamar</span>
                <span className="font-black text-indigo-600">Kamar {profileData?.room_number || "-"}</span>
              </div>
              <div className="w-full h-px bg-gray-50"></div>
              <div className="flex justify-between">
                <span className="text-gray-400">Periode Tagihan</span>
                <span className="font-bold text-gray-900">{tagihan?.periode_tagihan}</span>
              </div>
              <div className="w-full h-px bg-gray-50"></div>
              <div className="flex justify-between items-center">
                <span className="text-gray-400">Total Nominal</span>
                <span className="text-base font-black text-gray-900">{formatRupiah(tagihan?.nominal_tagihan || 0)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[24px] p-5 shadow-md border border-white/60 space-y-4">
            ...
            {/* FIX AXE: Hubungkan label dengan textarea keterangan */}
            <div className="space-y-1">
              <label htmlFor="input_keterangan" className="text-[10px] font-black text-gray-400 uppercase ml-0.5">Catatan Tambahan (Opsional)</label>
              <textarea
                id="input_keterangan"
                value={keterangan}
            ...
            {/* FIX AXE: Hubungkan label dengan input file upload struk */}
            <div className="space-y-2">
              <label htmlFor="input_bukti_file" className="text-[10px] font-black text-gray-400 uppercase ml-0.5">File Bukti Foto/Struk</label>
              <div className="relative border-2 border-dashed border-gray-200 rounded-2xl p-4 bg-gray-50 text-center hover:bg-gray-100/50 transition-colors cursor-pointer">
                ...
                {previewUrl ? (
                  ...
                  <div className="py-4 space-y-1 text-gray-400 pointer-events-none">
